...
Whole-branch review: complete (no Critical findings; four Important and two Minor findings were corrected, including sparse time-preset reset, fail-closed X-unit aggregation, lazy multi-logical-source identity, grouped checksum cancellation, error-text encoding, and frozen-font lesson scope)
Whole-branch review fix round 1: complete (full/unknown-cost adapters never escalate preview to a full load; metadata probes cache all descriptors once; unresolved preview/run/output/manifest identities remain stable; grouped checksum cancellation rewrites task/group/run/manifest truth)
Whole-branch review fix round 2: complete (deferred grouped terminal events and progress callbacks flush in original task order; full-cost HDF preview and unknown-cost registries remain no-load)
Whole-branch review fix round 3: complete (backend MDF probe failures translate to a narrow source-unavailable error; no-load preview falls back to unresolved identity while authoritative run loading and programming-error visibility are preserved)
Final independent re-review: PASS (25 targeted boundary tests; success, cancellation, member/group failure, blocked, resume, image ownership, metadata missing-path, and terminal ordering all verified)
Final verification: tests/test_batch_runner.py 199 passed; related manifest/output/source/source-adapter gate 118 passed with one recorded legacy file_paths baseline deselected; related UI gate 76 passed with the two recorded narrow-button and Windows QUrl baselines deselected; lessons gate clear; py_compile and git diff --check clean
