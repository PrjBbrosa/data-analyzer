...
class Icons:
    ...
    def ultraview_draw_pen(cls, color=None):
        ...
        def draw(p):
            ...
            p.setBrush(Qt.NoBrush)
            p.drawLine(QPointF(12.8, 3.6), QPointF(6.2, 12.8))
            p.drawLine(QPointF(16.0, 6.0), QPointF(9.4, 15.2))
            p.drawLine(QPointF(12.8, 3.6), QPointF(16.0, 6.0))
            ...
            nib.closeSubpath()
            p.drawPath(nib)
            p.drawLine(QPointF(7.0, 13.8), QPointF(4.2, 16.2))
            p.drawLine(QPointF(4.0, 16.8), QPointF(16.2, 16.8))
    ...
    def ultraview_draw_highlighter(cls, color=None):
        ...
        def draw(p):
            ...
            p.setBrush(Qt.NoBrush)
            barrel = QPainterPath()
            barrel.moveTo(7.4, 3.4)
            barrel.lineTo(14.2, 3.4)
            barrel.lineTo(13.7, 11.0)
            barrel.lineTo(6.9, 11.0)
            barrel.closeSubpath()
            p.drawPath(barrel)
            p.drawLine(QPointF(7.5, 5.3), QPointF(14.0, 5.3))
            chisel = QPainterPath()
            chisel.moveTo(5.6, 11.0)
            chisel.lineTo(15.0, 11.0)
            chisel.lineTo(15.6, 14.0)
            chisel.lineTo(5.0, 14.0)
            chisel.closeSubpath()
            p.drawPath(chisel)
            p.setPen(_pen(c, 2.1))
            p.drawLine(QPointF(3.8, 16.7), QPointF(16.2, 16.7))
    ...
    def ultraview_draw_eraser(cls, color=None):
        ...
        def draw(p):
            ...
            p.setBrush(Qt.NoBrush)
            body = QPainterPath()
            body.moveTo(3.4, 12.4)
            body.lineTo(10.0, 3.6)
            body.lineTo(16.6, 7.0)
            body.lineTo(10.0, 15.8)
            body.closeSubpath()
            p.drawPath(body)
            p.drawLine(QPointF(6.2, 13.6), QPointF(12.8, 5.6))
    ...
    def ultraview_draw_lasso(cls, color=None):
        ...
        def draw(p):
            ...
            loop = QPainterPath()
            loop.moveTo(7.6, 9.0)
            loop.cubicTo(4.2, 5.8, 7.2, 3.4, 11.4, 3.8)
            loop.cubicTo(15.6, 4.4, 16.4, 9.2, 14.4, 12.6)
            loop.cubicTo(12.4, 16.2, 6.8, 16.4, 5.0, 13.4)
            loop.cubicTo(3.8, 11.2, 4.8, 9.6, 7.6, 9.0)
            dash = _pen(c, 1.45)
            ...
            p.drawPath(loop)
            p.setPen(_pen(c, 1.45))
            p.drawLine(QPointF(7.6, 9.0), QPointF(16.4, 16.8))
            p.setBrush(QBrush(c))
            p.setPen(Qt.NoPen)
...
_PACKAGED_FALLBACK_RENDER_PX = 24
_PACKAGED_FALLBACK_DIR = Path(__file__).resolve().parent / "resources" / "qss_icons"


class IconFallbackResourceError(RuntimeError):
    """A mandatory, bundled QSS icon resource is absent or unreadable.

    This is deliberately not a cache failure. A frozen build without these
    static fallbacks is incomplete, so callers must allow it to surface rather
    than starting with invisible QSS subcontrols.
    """


class _IconCacheFilesystemError(RuntimeError):
    """Known filesystem failure while reading or writing the optional cache."""


def _qss_path(path: Path) -> str:
    """Return the forward-slash path Qt's Windows QSS parser requires."""
    return str(path).replace("\\", "/")


def _fallback_filename(placeholder: str) -> str:
    """Stable static-resource filename derived from an ``_ARROW_SPECS`` key."""
    return f"{placeholder.lower()}.png"


def _png_status(path: Path) -> str:
    """Classify one PNG without treating a nonempty file as automatically valid."""
    try:
        if not path.is_file():
            return "missing"
        ...
        raise _IconCacheFilesystemError(str(path)) from exc
...
def packaged_icon_fallback_paths() -> dict[str, str]:
    """Return validated, read-only fallbacks for every generated QSS glyph.

    The files are a release resource rather than a best-effort cache. They are
    generated by :func:`generate_packaged_icon_fallbacks` from
    ``_ARROW_SPECS`` and collected in both Windows flavors. A missing or
    corrupt file therefore raises a visible release-resource error so a smoke
    test cannot pass with unresolved/invisible QSS images.
    """
    paths: dict[str, str] = {}
    failures: list[str] = []
    seen: set[str] = set()
    for placeholder, _icon_name, _color in _ARROW_SPECS:
        if placeholder in seen:
            raise RuntimeError(f"duplicate QSS icon placeholder: {placeholder}")
        seen.add(placeholder)
        path = _PACKAGED_FALLBACK_DIR / _fallback_filename(placeholder)
        try:
            status = _png_status(path)
        except _IconCacheFilesystemError as exc:
            failures.append(f"{placeholder} ({exc.__cause__!r})")
            continue
        if status != "valid":
            failures.append(f"{placeholder} ({status}: {path})")
            continue
        paths[placeholder] = _qss_path(path)
    if failures:
        detail = "; ".join(failures)
        raise IconFallbackResourceError(
            "release-resource failure: packaged QSS icon fallback is missing "
            f"or invalid: {detail}",
        )
    return paths


def _cache_icon_path(
    out_dir: Path,
    icon_name: str,
    color: str,
    size_px: int,
    ...
    return out_dir / (
        f"{icon_name.replace('.', '_')}_{color_slug}_{size_px}_qta{qta_version}.png"
    )


def _cache_fallback_diagnostic(
    placeholder: str,
    stage: str,
    detail: object,
) -> None:
    """Record known cache degradation without allowing a retry log storm."""
    throttled(
        _LOGGER,
        f"qss_icon_cache_{stage}",
        logging.WARNING,
        "QSS icon cache fallback (%s) for %s; using packaged resource: %r",
        stage,
        placeholder,
        detail,
    )
...
def generate_packaged_icon_fallbacks(output_dir: Path | None = None) -> dict[str, Path]:
    """Generate static fallback PNGs directly from ``_ARROW_SPECS``.

    This maintenance-only helper is intentionally never called during app
    startup: production treats these as read-only release resources. Run it
    with the project Qt/qtawesome environment when updating a glyph definition,
    then commit the emitted ``mf4_analyzer/ui_kit/resources/qss_icons`` files.
    """
    app = QApplication.instance()
    if app is None:
        raise RuntimeError(
            "generate_packaged_icon_fallbacks() requires an existing QApplication",
        )
    import qtawesome as qta

    target_dir = Path(output_dir) if output_dir is not None else _PACKAGED_FALLBACK_DIR
    target_dir.mkdir(parents=True, exist_ok=True)
    generated: dict[str, Path] = {}
    for placeholder, icon_name, color in _ARROW_SPECS:
        pixmap = qta.icon(icon_name, color=color).pixmap(
            _PACKAGED_FALLBACK_RENDER_PX,
            _PACKAGED_FALLBACK_RENDER_PX,
        )
        if pixmap.isNull():
            raise IconFallbackResourceError(
                f"could not generate packaged QSS fallback for {placeholder}: null pixmap",
            )
        path = target_dir / _fallback_filename(placeholder)
        if not pixmap.save(str(path), "PNG"):
            raise IconFallbackResourceError(
                f"could not save packaged QSS fallback for {placeholder}: {path}",
            )
        if _png_status(path) != "valid":
            raise IconFallbackResourceError(
                f"generated packaged QSS fallback is not decodable: {path}",
            )
        generated[placeholder] = path
    return generated
...
def ensure_icon_cache():
    ...
    * Cache directory is ``~/.mf4-analyzer-cache/icons/``. PNG filenames
      ...
      manual invalidation.
    * Existing decodable PNGs are reused without rewriting (skip path).
    ...
    **Ordering constraint**: must be called AFTER ``QApplication`` has been
    ...
    # Validate the release resource first. Cache failures can be downgraded,
    # but a missing frozen/package fallback must fail loud instead of leaving
    # QSS's ``image: url(...)`` glyphs unresolved.
    fallback_paths = packaged_icon_fallback_paths()
    ...
    # used (the existing usage path). Its absence is an explicit optional
    # renderer/resource failure, not a reason to remove the QSS glyphs.
    try:
        import qtawesome as qta
    except ImportError as exc:
        for placeholder, _icon_name, _color in _ARROW_SPECS:
            _cache_fallback_diagnostic(placeholder, "qtawesome_import", exc)
    ...
    try:
        ...
        for placeholder, _icon_name, _color in _ARROW_SPECS:
            _cache_fallback_diagnostic(placeholder, "cache_directory", exc)
        return fallback_paths
    paths = {}
    generated = 0
    ...
    for placeholder, icon_name, color in _ARROW_SPECS:
        ...
        try:
            status = _png_status(out_path)
        except _IconCacheFilesystemError as exc:
            _cache_fallback_diagnostic(placeholder, "cache_read_io", exc.__cause__)
            paths[placeholder] = fallback_paths[placeholder]
            continue

        if status == "valid":
            # The healthy-cache path deliberately leaves bytes and timestamps
            # untouched so current visuals and cache-hit behavior are stable.
            paths[placeholder] = _qss_path(out_path)
            continue
        if status != "missing":
        ...
        pix.setDevicePixelRatio(ratio)
        try:
            saved = pix.save(str(out_path), "PNG")
        except OSError as exc:
            _cache_fallback_diagnostic(placeholder, "cache_write_io", exc)
            paths[placeholder] = fallback_paths[placeholder]
            continue
        if not saved:
            _cache_fallback_diagnostic(placeholder, "cache_save_false", out_path)
            paths[placeholder] = fallback_paths[placeholder]
            continue
        try:
            saved_status = _png_status(out_path)
        except _IconCacheFilesystemError as exc:
            _cache_fallback_diagnostic(placeholder, "cache_write_io", exc.__cause__)
            paths[placeholder] = fallback_paths[placeholder]
            continue
        if saved_status != "valid":
            _cache_fallback_diagnostic(placeholder, "cache_decode", saved_status)
            paths[placeholder] = fallback_paths[placeholder]
            continue
