...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "总览画布右键",
                sub="空白处：适应内容、100%、概览、智能排版（重选大小与位置，可撤销）、紧凑排列（保留尺寸只消空洞）、复制图片、导出 PNG。锁定卡片占用空间时布局未改变；无法排入时可用等大网格完成降级排版。按原图比例在卡片右上角，只收紧单卡外壳",
            ),
        ),
    ),
    QuickGroup(
