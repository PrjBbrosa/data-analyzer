    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "固定当前读数",
                keys=(_sc("pin_cursor"),),
                sub="仅时域 / 频谱 / 频响数据图的现有可用上下文，不必先点。也可点读数上的 P。新 Pin 默认收起；展开后默认 −，点 + 会记住。点图底 Pn 各自展开/收起，已展开的 Pn 有底色。左右拖动或焦点下 ←/→ 调位置，Shift 微调。总览 Board 与图片标注里 P 仍是画笔。",
            ),
            QuickRow(
                "平移 / 框选模式",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "固定读数面板",
                keys=(_sc("pin_cursor"),),
                sub="单游标按 P 或点读数 P 固定；双游标先点 A、B 再 P。新 Pin 默认收起；展开默认 −，点 + 记住。点图底 Pn 各自展开/收起，展开有底色；拖动或焦点下 ←/→，Shift 微调。关游标不隐藏已固定。蓝图钉取消固定并原地接回；× 关闭；面板 −/+ 仍切换 full/mini。",
            ),
            QuickRow(
                "游标显示设置",
            ...
            QuickRow(
                "频谱 / 频响游标",
                sub="工具栏选关 / 单 / 双。频谱与时域共用表格：− 隐名称，空间不足显示 +N，放大后恢复。双游标读 A/B 与 Δf；频响仍突出 ΔY。每个 pane 记住，默认关闭",
            ),
        ),
    ),
    # 8 — 谱图（FFT-时间 / 阶次）
