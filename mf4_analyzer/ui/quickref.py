...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "总览",
                sub="独立面板，只读对照已有预览：不计算；左上 Board 菜单管理多个 Board，左侧窄轨打开 View 库、布局、筛选和未放置；右上浮岛开关标题/来源、复制导出和演示；右下浮岛控制概览与缩放；停手后跟上图面，含游标读数与标注",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "画布缩放 / 平移",
                sub="Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–200%；空格+拖或中键拖平移；工具栏 − / % / ＋ / 适应 / 100%；双击卡片铺满视口。整板概览仍可点卡片跳转",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "缩略图 / 整板概览",
