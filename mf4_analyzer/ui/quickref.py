...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "自由网格",
                sub="左侧窄轨按钮切换；12 列、最多 24 张卡；直接拖动移动，选中后拖边角改尺寸；空白框选、Shift 点选多张；拖动预览最终落点与被推移卡（完整 ghost）；移动不改任何卡尺寸、缩放只改本卡；放不下则拒绝并保持原布局，不弹框",
                keys=("Shift",),
            ),
            QuickRow(
                "键盘微调",
            ...
            QuickRow(
                "尺寸预设",
                sub="小 / 标准 / 宽 / 高 / 大 / 横幅；放不下则拒绝并保持原布局，不静默改邻卡",
                gesture="卡片右键",
            ),
            QuickRow(
                "撤销 / 重做",
            ...
            QuickRow(
                "画布缩放 / 平移",
                sub="Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–200%；空格+拖或中键拖平移；工具栏 − / % / ＋ / 适应 / 100%。适应把卡片停在浮层安全区，缩放可伸到工具栏下；双击卡片铺满视口，已铺满再双击打开临时查看（不改变源 View）。≥60% 完整卡片，40%–59% 紧凑（藏页脚、留预览与类型），低于 40% 标题卡。整板概览仍可点卡片跳转",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "源已变化",
