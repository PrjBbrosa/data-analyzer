...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            QuickRow(
                "切换时保留坐标",
                sub="频谱、时频和阶次预设将覆盖手动范围时，可选择保留手动范围、使用预设范围或取消；仅本次有效。幅值单位或 dB 参考改变时，对应幅值范围自动调整。蓝色高亮表示当前调整基准，不是完全匹配；紫点为参数差异，黄点为相对目标预设的坐标差异。保留手动坐标后仍以目标预设为基准，同槽可再应用。提示「槽位已更新」或「来源版本未知」时可再加载，不会当成无操作。",
