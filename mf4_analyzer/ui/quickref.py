...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "启动面板",
                sub="启动时可能出现 TraceLab 启动面板，表示程序正在启动；等待时随机显示一条操作提示。启动面板先关闭，再显示主窗口。它不是项目加载进度，也不是分析百分比。",
            ),
            QuickRow(
                "支持格式",
            ...
            QuickRow(
                "回放对齐提示",
                sub="已按公共时间轴对齐的 MF4 会在 Play 前显示只读说明，可能包含非原始测量值。换无警告文件会清除；加载失败时保留上一份文件的说明。",
            ),
            QuickRow(
                "WWT WinWert 视图",
            ...
            QuickRow(
                "保存会话",
                sub="主按钮保存当前工程；箭头展开另存为与关闭项目。关闭会先保护未保存内容并解除旧保存路径，之后保存需另存为。有未保存更改时可保存、不保存或取消。",
                gesture="工具栏「保存」",
            ),
            QuickRow(
                "关闭项目",
                sub="结束当前工程并保持窗口打开。移除最后一个文件可选择保留空项目，后续保存仍写原路径；关闭项目才会解绑。",
                gesture="工具栏「项目操作」或文件区「关闭项目…」",
            ),
            QuickRow(
                "操作速查",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "有效事实卡",
                sub="计算后显示实际 Fs / NFFT / Δf / 窗长 / 帧数；时间轴重建仅用于分析，时域保留原始时间。自动优先 4096 点；低 Fs 按窗长适配。点数缩短与统计不足分开提示，零填充不会提高物理分辨率。",
            ),
            QuickRow(
                "自动 NFFT",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "分屏平移 / 框选",
                sub="同一分区两个可见窗格同步鼠标模式；是否连动坐标范围仍由联动开关决定。隐藏分区不受影响。",
                gesture="工具栏、右键或快捷键",
            ),
            QuickRow(
                "图表视图后退 / 前进",
                sub="各可见窗格使用自己的进程内历史，刚完成的缩放可立即后退。时频与阶次回退主图范围并带动切片。不写入项目。",
                gesture="工具栏后退 / 前进",
            ),
            QuickRow(
                "拖通道加入当前 View",
            ...
            QuickRow(
                ...
                sub="平移·Shift缩Y·Ctrl缩X；预览缩放只查看，不改计算范围；轴槽单通道；设左轴",
            ),
            QuickRow(
                "编辑某曲线颜色/坐标（叠加）",
                gesture="双击曲线或其 Y 轴",
            ...
            QuickRow(
                "分析图缩放保持",
                sub="频谱 / 时频 / 阶次按轴保存缩放；Home 查看全部，不修改参数面板的范围设置。再次点击计算后按当前参数范围显示。",
            ),
            QuickRow(
                "时域时间范围",
                sub="全时段用完整数据，数字仍随视窗联动且只读；指定范围才是下次绘图的请求。切换不绘图、不 Home。图表 Home / 查看全部仍是图形入口。",
                gesture="全时段 | 指定范围",
            ),
            QuickRow(
                "分析时间范围",
                sub="全时段或指定开始/结束决定下次计算。切换不重算、不 Home。覆盖不足时保留指定范围，不静默退回全时段。分析缩放只查看。未写完或倒序起止保留原文。",
                gesture="全时段 | 指定范围",
            ),
            QuickRow(
                "视角后退 / 前进",
            ...
            QuickRow(
                "时域 View",
                sub="最多 24 个；单图切换时新图自然绘制就绪后会短暂淡入，不改变数据、坐标或游标取值；分屏与项目恢复仍直接到达终态；窄窗口先显示编号，悬停看全名；「⋯」始终可管理全部 View，变为「»N」时 N 是未显示的 View 数；面板可逐项关闭、关闭其他或关闭全部",
            ),
            QuickRow(
                "当前 View 的绘图意图",
                sub="共轴、滤波、显式颜色和图表选项外观（标题、轴标签、对数、网格）跟当前时域 View：切换、复制、分屏和重开后互不影响。横坐标标签仍走自定义 X。",
            ),
            QuickRow(
                "图表选项",
                sub="显示当前目标。共享 X 时标题与网格属于整张图，X 为共享轴，Y 与曲线属于当前轴。空标题收回占位。还原立即恢复本次打开时的图表。热图不能开对数。频响暂不支持图表选项。",
            ),
            QuickRow(
                "分析页 View",
                sub="频谱 / 时频 / 阶次 / 频响单图切换与跨区导航，目标就绪后同样淡入；未访问过的分析页会在空闲时自动准备，点未就绪页时优先准备并可切走；分屏、未计算页和项目恢复仍直接到达终态",
            ),
            QuickRow(
                "View 标签关闭",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "固定当前读数",
                keys=(_sc("pin_cursor"),),
                sub="仅时域 / 频谱 / 频响数据图的现有可用上下文，不必先点。也可点读数上的 P。新 Pin 默认收起；展开后默认数值，可改完整。点图底 Pn 各自展开/收起，已展开的 Pn 有底色。左右拖动或焦点下 ←/→ 调位置，Shift 微调。总览 Board 与图片标注里 P 仍是画笔。",
            ),
            QuickRow(
                "平移 / 框选模式",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "搜索通道",
                sub="关键词与「已选」取交集。空态「清除筛选」同时清关键词和已选，并恢复筛选前展开与滚动；不恢复勾选或眼睛显隐。Esc 有词时只清关键词。",
                gesture="通道树搜索框",
            ),
            QuickRow(
                "WinWert 原始辅助线",
            ...
            QuickRow(
                "合并为共轴比幅值",
                sub="Ctrl+单击不连续多选；Shift+单击连续范围。只作用于当前时域 View。分屏共轴图例每条曲线一行，文字跟随曲线颜色。",
                gesture="多选后右键",
            ),
            QuickRow(
                "看通道全名",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "搜索下拉选项",
                gesture="在下拉框中输入关键词",
                sub="分析信号、转速、频响输入/输出、自定义横坐标和派生通道选择框均可搜索通道；通道配置框可搜索配置名称。也可点击右侧箭头选择。",
            ),
            QuickRow(
                "单通道运算",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "固定读数面板",
                keys=(_sc("pin_cursor"),),
                sub="单游标按 P 或点读数 P 固定；双游标先点 A、B 再 P。新 Pin 默认收起；展开默认数值，可改完整。点图底 Pn 各自展开/收起，展开有底色；拖动或焦点下 ←/→，Shift 微调。关游标不隐藏已固定。固定面板不显示 P 按钮；× 删除该 Pn；收起请点图底 Pn。",
            ),
            QuickRow(
                "游标显示设置",
                sub="独立开关最小值点、最大值点及 Min / Max / Avg / 差值，全局同步分屏。单/双游标短名与值同行，窄时分行；Custom-X 方向/状态同表。X↑/X↓ 各算 Y(B)−Y(A)，端点越界显示 —。数值 / 完整切换密度：数值隐藏名称，完整显示；读数不弹出 tooltip。",
            ),
            QuickRow(
                "频谱 / 频响游标",
                sub="工具栏选关 / 单 / 双。频谱数值隐名称；频响数值保留 |H|、φ、γ² 短标签，双游标数值看 Δ，仅 A 时等待 B。空间不足时频谱显示 +N channels，频响显示 +N 项指标，放大后恢复。双游标读 A/B 与 Δf。每个 pane 记住，默认关闭",
            ),
        ),
    ),
    # 8 — 谱图（FFT-时间 / 阶次）
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "切片跟随谱图视野",
                sub="切片始终跟随主图当前时间 / 频率 / 阶次范围；Home 同步查看全部",
                gesture="缩放 / 平移 / Home",
            ),
    ...
    QuickGroup(
        ...
        rows=(
            QuickRow(
                "开启标注",
                sub="时域、频谱、时频、频响、阶次均可用；分屏时只作用于当前焦点窗格，另一侧不变",
                gesture="工具栏按钮",
            ),
            QuickRow("添加 / 删除最近", gesture="左键加 · 右键删"),
            QuickRow(
                "清除当前焦点图表标注",
                sub="确认期间若焦点图表被销毁或所属 View 已切换，则取消，不清除新目标",
                gesture="工具栏橡皮擦按钮",
            ),
        ),
    ),
    ...
    QuickGroup(
        ...
        rows=(
            QuickRow(
                "切换时保留坐标",
                sub="预设覆盖手动范围时，可选保留坐标、使用预设范围或取消，仅本次有效。幅值单位或 dB 参考改变时，幅值范围随之调整。蓝色表示当前基准；紫点为参数差异，黄点为坐标差异。保留坐标后仍以目标预设为基准，可再次应用同槽；「槽位已更新」或「来源版本未知」时也可重载。",
                gesture="切换预设 / 再次点击当前预设重新应用",
            ),
            QuickRow("保存当前分析参数", gesture="预设槽"),
            QuickRow(
                "重命名 / 重置槽位 / 恢复面板默认",
                sub="重置此槽为内置只改槽内容；恢复面板默认参数才清空基准并套用构造默认。",
                gesture="右键预设槽",
            ),
        ),
    ),
    ...
    QuickGroup(
        ...
        rows=(
            QuickRow(
                "复制为图片",
                sub="含游标线和读数，可标注。保存图片使用同一张合成图，包含可见读数与标签。",
                gesture="复制按钮",
            ),
            QuickRow(
                "导出 Excel / WWT",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "先选分析方法",
                sub="首次打开默认为时域；时域 / 频谱 / 时频 / 阶次 / 频响；浅蓝栏内渐变浅底与蓝色边框标记当前方法，目标与参数随方法切换。Tab 聚焦，左右方向键切换",
                gesture="顶部方法行",
            ),
            QuickRow(
                "方法对应目标",
                sub="时域、频谱、时频选分析信号；阶次再指定转速通道和系数；频响做输入与输出配对",
            ),
            QuickRow(
                "有效目标数量",
                sub="分组卡、底栏和预览用同一次规划结果。来源通道不齐时按实际可算项计数，不是来源数×信号数。来源未就绪或频响未配完显示「待确定」，不填看起来精确的数字。关闭图片导出时写分组示意和 0 张实际图片。",
            ),
            QuickRow(
                "图内布局",
                sub="每项单独时叠加/分屏禁用但保留原值，原因在字段标签；按数据源或按信号合并后可改。运行结束仍按分组条件恢复。变灰的选项仍保留原值，不是被清成默认。",
            ),
            QuickRow(
                "频谱字段适用性",
                sub="单帧禁用窗长与平均重叠；固定 NFFT 时窗长由 NFFT 与采样率决定。普通重叠率只在时频和频响显示",
            ),
            QuickRow(
                "去添加 / 去选择 / 去检查",
                sub="底栏文字入口只滚动到当前问题并聚焦，不改配置、不自动打开对话框",
                gesture="底栏",
            ),
            QuickRow(
                "添加来源",
                sub="已加载菜单实时读取主窗口文件；从磁盘选择文件，或点右侧箭头搜索最近数据文件",
                gesture="+ 已加载 / + 从磁盘… / 右侧下拉箭头",
            ),
            QuickRow(
                "选目标信号",
            ...
            QuickRow(
                "线性幅值与 dB 参考",
                sub="Linear 时整组 dB 参考禁用，不改参考模式和数值；切回 dB 后恢复。时域和频响仍隐藏该行",
            ),
            QuickRow(
                "数据导出默认关闭",
                sub="所有方法默认不勾选数据文件；勾选后，预计较大的 XLSX 会在运行前显示红色提示。提示按完整数据估算，不阻止运行",
            ),
            QuickRow(
                "仅导出数据",
                sub="取消图片但保留 XLSX 时，合并、布局、样式和显示范围仅用于预览，不改表格数值。计算截取、滤波和切片仍影响数据",
            ),
            QuickRow(
                "导出切片",
            ...
            QuickRow(
                "运行警告",
                sub="结果区汇总本次运行警告；查看详情里该行自己的警告单独列出。加载对齐留在该行，series=original 仍只表示未经用户滤波。",
            ),
            QuickRow(
                "查看详情",
                sub="运行结束后底栏展开本次逐项原因、警告和产物；Esc 先收起并把焦点交还入口，不关闭批处理。可检查输出目录或频响配对，不自动重试。",
                gesture="底栏「查看详情」",
            ),
            QuickRow(
                "记住导出偏好",
