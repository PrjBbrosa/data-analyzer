...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "画布缩放 / 平移",
                sub="每次打开或切换 Board 都自动适应内容。Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–300%；空格+拖、中键拖或手掌四向平移；拖到边缘自动平移并扩展画布。工具栏 − / % / ＋ / 适应 / 100%。12 列基准网格与导出同口径，缩放不改变卡片长宽比。适应内容把已放置卡片居中填满浮层安全区，最高 300%；100% 保持当前中心。双击卡片临时聚焦最高 300%，Esc 返回。≥60% 完整卡片，40%–59% 紧凑（藏页脚、留预览与类型），低于 40% 标题卡。整板概览仍可点卡片跳转。画布空白右击可适应、自动排版、复制或导出",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "源已变化",
