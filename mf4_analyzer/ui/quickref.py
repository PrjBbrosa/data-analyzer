...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "总览",
                sub="独立面板，只读对照已有预览：不计算；左上 Board 弹层管理多个 Board，左侧窄轨打开 View 库、自由网格、布局、筛选和未放置；View 库可钉住以免点画布时收起；右上浮岛开关标题/来源、卡片操作常驻、复制导出和演示；取消卡片操作常驻后，悬停或键盘聚焦卡片才显示操作；右下浮岛控制概览与缩放；停手后跟上图面，含游标读数与标注",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "总览卡片右键",
                sub="打开原 View · 同步到最新 · 复制本卡图像 · 移到未放置 · 从当前 Board 移除（不删除源 View）；右上角胶囊可直接打开/聚焦/适应/移除/更多，过期才出现同步；复制、未放置和重绑仍在更多和右键里",
            ),
        ),
    ),
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "自由网格",
                sub="新 Board 默认自由网格。左侧窄轨按钮切回模板；12 列是基准网格和导出标尺，不是日常拖动墙。工作区随内容扩展，四向可平移；拖到边缘自动平移并扩展画布。最多 24 张已放置、200 个 View。整数网格；直接拖动移动，选中后拖边角改尺寸；空白框选、Shift 点选多张；拖动预览最终落点与被推移卡（完整 ghost）；移动不改任何卡尺寸、缩放只改本卡；硬拒绝会说明原因和下一步，不弹框",
                keys=("Shift",),
            ),
            QuickRow(
                "键盘微调",
            ...
            QuickRow(
                "画布缩放 / 平移",
                sub="每次打开或切换 Board 都自动适应内容。Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–300%；空格+拖、中键拖或手掌四向平移；拖到边缘自动平移并扩展画布。工具栏 − / % / ＋ / 适应 / 100%。12 列基准网格与导出同口径，缩放不改变卡片长宽比。适应内容把已放置卡片居中填满浮层安全区，最高 300%；100% 保持当前中心。双击卡片临时聚焦最高 300%，Esc 返回。≥60% 完整卡片，40%–59% 紧凑（藏页脚、留预览与类型），低于 40% 标题卡。整板概览仍可点卡片跳转",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "源已变化",
