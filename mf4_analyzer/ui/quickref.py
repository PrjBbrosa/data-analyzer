...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "时域「全部」",
                sub="X 轴回到图面已绘通道的最长全程，不是全局最长文件；不勾选过滤。Home 同样回已绘范围。",
                gesture="Inspector「全部」",
            ),
            QuickRow(
                "分析页「全部」",
                sub="取消勾选、清除待启用草稿，显示当前来源全时段。分析缩放只查看，不改计算范围；手动改起止是待启用草稿，未勾选用全时段，点计算才询问。未写完或倒序的起止会保留原文并提示，勾选不会改成全时段；只有点「全部」或取消启用才放弃。越界范围不能当局部计算，可改回、用全时段或取消。",
                gesture="Inspector「全部」",
            ),
            QuickRow(
                "视角后退 / 前进",
