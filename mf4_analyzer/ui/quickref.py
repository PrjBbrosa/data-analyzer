...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "总览",
                sub="独立面板，只读对照已有预览：不计算；左上 Board 弹层管理多个 Board，左侧窄轨打开 View 库、自由网格、布局、筛选和未放置；View 库可钉住以免点画布时收起；右上浮岛开关标题/来源、复制导出和演示；右下浮岛控制概览与缩放；停手后跟上图面，含游标读数与标注",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "画布缩放 / 平移",
                sub="新 Board 初始缩放不超过 66%。Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–300%；空格+拖、中键拖或手掌四向平移；拖到边缘自动平移并扩展画布。工具栏 − / % / ＋ / 适应 / 100%。12 列基准网格与导出同口径，缩放不改变卡片长宽比。适应内容只缩不放、最高 100%，把已放置卡片居中放进浮层安全区；100% 保持当前中心。双击卡片临时聚焦最高 300%，Esc 返回。≥60% 完整卡片，40%–59% 紧凑（藏页脚、留预览与类型），低于 40% 标题卡。整板概览仍可点卡片跳转",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "源已变化",
