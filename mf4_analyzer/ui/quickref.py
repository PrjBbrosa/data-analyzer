...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "当前 View 的绘图意图",
                sub="共轴、滤波、显式颜色和图表选项外观（标题、轴标签、对数、网格）跟当前时域 View：切换、复制、分屏和重开后互不影响。横坐标标签仍走自定义 X。图例是一次性重新生成，不保存。",
            ),
            QuickRow(
                "图表选项",
                sub="显示当前目标。共享 X 时标题与网格属于整张图，X 为共享轴，Y 与曲线属于当前轴。空标题收回占位。重置恢复打开时设置。图例只重新生成且不保存。热图不能开对数。频响暂不支持图表选项。",
            ),
            QuickRow(
                "分析页 View",
