...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "便签 Sticky",
                sub="左侧创作段 V / N / T / S / P。N 单击打开 16 色便签，选色一次放置，底部 Stack 连续放置。点空白创建并立即编辑，拖框按起止尺寸创建。空内容退出不保存、不进撤销；有内容后 Ctrl/Cmd+Enter 提交，Esc 取消。不计算、不改源 View",
                keys=("N",),
            ),
            QuickRow(
                "文字 Text",
                sub="T 进入文字，不弹空层。点空白创建自适应宽文本框并立即编辑，拖动先定换行宽度。中文输入法组合期间快捷键不抢。选中后贴对象的图标栏整框改字体/字号/B/I/U/对齐/列表/颜色/底色/链接，不伪装局部富文本。放置一次回选择；双击按钮才连续。空内容退出不保存、不进撤销；非空提交一条历史。上限 6000 字",
                keys=("T",),
            ),
            QuickRow(
                "形状 Shape",
                sub="S 单击打开形状与连接线：直线、箭头、折线与矩形/圆角矩形/椭圆/菱形/三角同层选择。L 直达最近连接线。点空白创建默认 4×3，拖动定尺寸；Shift 保持比例，Alt 从中心，Cmd/Ctrl 暂停吸附。选中后图标栏改填充/描边/线宽/虚线/圆角。双击或选中后输入编辑内嵌文字。P 打开钢笔/荧光笔/整笔擦除/套索、线宽预览和颜色",
                keys=("S",),
            ),
            QuickRow(
                "已有连线与笔画",
