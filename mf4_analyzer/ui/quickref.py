...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "画布缩放 / 平移",
                sub="新板默认适应视口。Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–300%；空格+拖或中键拖平移；工具栏 − / % / ＋ / 适应 / 100%。逻辑画布与导出同口径，缩放不改变卡片长宽比。适应把已放置卡片居中放进浮层安全区，缩放可伸到工具栏下；双击卡片铺满视口，已铺满再双击打开临时查看（不改变源 View）。≥60% 完整卡片，40%–59% 紧凑（藏页脚、留预览与类型），低于 40% 标题卡。整板概览仍可点卡片跳转",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "源已变化",
