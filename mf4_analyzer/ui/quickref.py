...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "自由网格",
                sub="新 Board 默认自由网格。左侧窄轨按钮切回模板；12 列、最多 24 张卡；逻辑画布固定 1600 宽，缩放不改卡片长宽比。直接拖动移动，选中后拖边角改尺寸；空白框选、Shift 点选多张；拖动预览最终落点与被推移卡（完整 ghost）；移动不改任何卡尺寸、缩放只改本卡；放不下则拒绝并保持原布局，不弹框",
                keys=("Shift",),
            ),
            QuickRow(
                "键盘微调",
            ...
            QuickRow(
                "尺寸预设",
                sub="小 / 标准 / 宽 / 高 / 大 / 横幅，或「按原图比例」按预览图宽高比吸附格数；放不下则拒绝并保持原布局，不静默改邻卡。模板模式下该操作不可用",
                gesture="卡片右键 / 选中条",
            ),
            QuickRow(
                "撤销 / 重做",
                sub="直接移动或调整后可使用 Ctrl+Z / Ctrl+Shift+Z 恢复或重做；一次重排对应一次撤销，可恢复全部受影响卡片",
                keys=("Ctrl+Z", "Ctrl+Shift+Z"),
            ),
            QuickRow(
                "画布缩放 / 平移",
                sub="新板默认适应视口。Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–200%；空格+拖或中键拖平移；工具栏 − / % / ＋ / 适应 / 100%。逻辑画布与导出同口径，缩放不改变卡片长宽比。适应把卡片停在浮层安全区，缩放可伸到工具栏下；双击卡片铺满视口，已铺满再双击打开临时查看（不改变源 View）。≥60% 完整卡片，40%–59% 紧凑（藏页脚、留预览与类型），低于 40% 标题卡。整板概览仍可点卡片跳转",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "源已变化",
