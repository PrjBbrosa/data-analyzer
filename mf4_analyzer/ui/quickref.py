...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "WWT WinWert 排版",
                sub="多窗口同步到以文件名命名的独立 Board；单窗口只生成时域 View，不自动加入 UltraView。一次打开多个 WWT 时可勾选「对本次剩余 WWT 使用此选择」，只作用于这一次打开/拖入，下次会重新询问。WinWert View 的查看全部/Home 回到文件原生 X 图面（例如 -100..100 mm），不是数据并集。导入完成会报告已放置/未放置数量。公式警告「未生成」不等于整个文件导入失败；成功自动调整位置不会出黄条。不支持的公式或显示选项会警告，不是像素级复刻",
            ),
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "总览画布右键",
                sub="空白处：适应内容、100%、概览、智能排版（重选大小与位置，可撤销）、紧凑排列（保留尺寸只消空洞）、复制图片、导出 PNG。按原图比例在卡片右上角，只收紧单卡外壳",
            ),
        ),
    ),
    QuickGroup(
