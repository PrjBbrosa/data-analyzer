...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "分析页「全部」",
                sub="取消勾选、清除待启用草稿，显示当前来源全时段。分析缩放只查看，不改计算范围；手动改起止是待启用草稿，未勾选用全时段，点计算才询问。",
                gesture="Inspector「全部」",
            ),
            QuickRow(
                "视角后退 / 前进",
    ...
    QuickGroup(
        ...
        rows=(
            QuickRow(
                "切换时保留坐标",
                sub="频谱、时频和阶次预设将覆盖手动范围时，可选择保留手动范围、使用预设范围或取消；仅本次有效。幅值单位或 dB 参考改变时，对应幅值范围自动调整。蓝色高亮表示当前调整基准，不是完全匹配；紫点为参数差异，黄点为坐标差异。",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "图内布局",
                sub="每项单独时叠加/分屏禁用但保留原值，原因在字段标签；按数据源或按信号合并后可改。运行结束仍按分组条件恢复",
            ),
            QuickRow(
                "频谱字段适用性",
