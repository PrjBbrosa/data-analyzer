...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "总览卡片右键",
                sub="打开原 View · 同步到最新 · 复制卡片 · 移入托盘 · 移除；选中条常驻打开/同步/聚焦/更多，其余在更多里",
            ),
        ),
    ),
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "自由网格",
                sub="左侧窄轨按钮切换；12 列、最多 24 张卡；直接拖动移动，选中后拖边角改尺寸；空白框选、Shift 点选多张；重叠时邻卡按原尺寸平移让位并显示完整 ghost，边缘放不下则保持原布局；整理只压缩空行",
                keys=("Shift",),
            ),
            QuickRow(
                "键盘微调",
            ...
            QuickRow(
                "画布缩放 / 平移",
                sub="Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–200%；空格+拖或中键拖平移；工具栏 − / % / ＋ / 适应 / 100%。适应把卡片停在浮层安全区，缩放可伸到工具栏下；双击卡片铺满视口。整板概览仍可点卡片跳转",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "源已变化",
