...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "WWT WinWert 排版",
                sub="多窗口同步到以文件名命名的独立 Board；单窗口只生成时域 View，不自动加入 UltraView。一次打开多个 WWT 时可勾选「对本次剩余 WWT 使用此选择」，只作用于这一次打开/拖入，下次会重新询问。WinWert View 的查看全部/Home 回到文件原生 X 图面（例如 -100..100 mm），不是数据并集。导入完成会报告已放置/未放置数量。导入后的一次智能排版可用撤销恢复；保存项目后重开不会自动重排。公式警告「未生成」不等于整个文件导入失败；成功自动调整位置不会出黄条。不支持的公式或显示选项会警告，不是像素级复刻",
            ),
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "拖通道设为横坐标",
                sub="拖到图底 X 带即可替换当前横坐标；按各来源匹配同名通道，失败只占位/跳过该来源，不回滚成时间轴",
                gesture="拖到最底部 X 带",
            ),
            QuickRow(
                "单独调某通道 Y（叠加）",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "双游标",
                sub="点 A 点 B。时间轴显示 ΔT 与 1/ΔT；Custom X 显示物理单位与 ΔX，每条曲线按 X↑/X↓ 分行统计，不伪造单值 △Y。无法可靠区分时会给出诊断",
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "总览卡片右键",
                sub="替换为… · 移到未放置 · 复制本卡图像 · 尺寸预设；过期才出现同步。打开、聚焦、按原图比例和移除在右上角胶囊。更多菜单可锁定 / 解锁，锁定后后续智能排版保留位置",
            ),
            QuickRow(
                "总览画布右键",
                sub="空白处：适应内容（只缩放画布，不改卡片）、100%、概览、智能排版（重新计算大小与位置，可撤销）、紧凑排列（保留尺寸只消空洞）、复制图片、导出 PNG。布局已是最优状态时智能排版不写撤销。锁定卡片占用空间时布局未改变；无法排入时可用等大网格完成降级排版。按原图比例在卡片右上角，只收紧当前卡",
            ),
        ),
    ),
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "指针",
                sub="自由网格创作段首位。整块单击打开鼠标/激光笔两行；不会直接应用上次模式。两者都可选择、移动、缩放；激光笔仅换成中心发光圆点光标。切换模式会保留当前选择与控制点。V 进入或保持选择工具但不改光标样式；Esc 先取消编辑/草稿/弹层，再清除选择，亦不改光标样式。空间键和中键仍可平移",
                keys=("V", "Esc"),
            ),
            QuickRow(
                "便签 Sticky",
                sub="左侧创作段指针 / N / T / S / P。按 V 或 Esc 回到鼠标指针。N 单击打开 16 色便签，选色一次放置，底部 Stack 连续放置。点空白创建并立即编辑，拖框按起止尺寸创建。四角缩放靠近正方形时吸附，Cmd/Ctrl 旁路。空内容退出不保存、不进撤销；有内容后 Ctrl/Cmd+Enter 提交，Esc 取消。不计算、不改源 View",
                keys=("N",),
            ),
            QuickRow(
                "文字 Text",
                sub="T 进入文字，不弹空层。点空白创建自适应宽文本框并立即编辑，拖动先定换行宽度。中文输入法组合期间快捷键不抢。选中后贴对象的图标栏整框改字体/字号/B/I/U/对齐/列表/颜色/底色，不伪装局部富文本。放置一次回选择；双击按钮才连续。空内容退出不保存、不进撤销；非空提交一条历史。上限 6000 字。V 或 Esc 回到选择",
                keys=("T",),
            ),
            QuickRow(
                "形状 Shape",
            ...
            QuickRow(
                "多选 · 对齐",
                sub="Shift 点选或框选多个作者对象后可左/中/右/上/中/下对齐，以及水平/垂直分布。卡片与作者混选只保留移动、删除、锁定，不做危险批量样式。锁定和未知对象不改内容、不从板上丢掉。Ctrl/Cmd+D 复制，Ctrl/Cmd+C/V 按类型粘贴并分配新 id",
                keys=("Ctrl+D", "Shift"),
            ),
            QuickRow(
                "键盘微调",
            ...
            QuickRow(
                "锁定卡片",
                sub="卡片更多菜单「锁定」后，后续智能排版保留其位置；再选一次即可解锁。锁定卡片占用空间时布局未改变。保存项目后重开不会自动重排",
                gesture="卡片更多菜单",
            ),
            QuickRow(
                "替换卡片",
            ...
            QuickRow(
                "尺寸预设",
                sub="小 / 标准 / 宽 / 高 / 大 / 横幅在右键尺寸预设；「按原图比例」在右上角，按当前预览尺度只收紧当前卡去贴图像宽高比，去掉上下或左右留白，不整板重选一张更大或更小的卡；放不下则拒绝并保持原布局，不静默改邻卡。模板模式下该操作不可用",
                gesture="卡片右键 / 右上角",
            ),
            QuickRow(
                "撤销 / 重做",
                sub="直接移动、调整或从当前 Board 移除后可使用 Ctrl/Cmd+Z / Ctrl+Shift+Z 恢复或重做；移除不删除源 View。一次重排对应一次撤销，可恢复全部受影响卡片。WWT 导入后的一次智能排版也可撤销",
                keys=("Ctrl+Z", "Ctrl+Shift+Z"),
            ),
            QuickRow(
                "画布缩放 / 平移",
                sub="每次打开或切换 Board 都自动适应内容。Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–300%；左键按住空白框选，右键按住拖动画布；空格+拖、中键拖或手掌也可四向平移；拖到边缘自动平移并扩展画布。工具栏 − / % / ＋ / 适应 / 100%。12 列基准网格是卡片 1× 标尺；导出 PNG 按已放置内容裁切，与适应内容同口径，缩放不改变卡片长宽比。适应内容只缩放画布、不改卡片，把已放置卡片居中填满点阵画布，最高 300%；100% 保持当前中心。双击卡片临时聚焦最高 300%，Esc 返回。≥60% 完整卡片，40%–59% 紧凑（藏页脚、留预览与类型），低于 40% 标题卡。整板概览仍可点卡片跳转。画布空白右击可适应内容、智能排版、紧凑排列、复制或导出",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "源已变化",
                sub="预览与当前图面不一致（缩放、游标、标注或结果）。点同步抓取最新画面，不重新计算。左侧工具栏「一键更新源」可一次更新全部已变化预览。分辨率偏低时打开源 View 后可更新；源 View 未打开会提示「预览分辨率较低，打开源 View 后可更新」，不会触发智能排版",
