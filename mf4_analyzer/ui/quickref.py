...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "便签 Sticky",
                sub="左侧创作段 N / T / S / P。按 V 或 Esc 回到选择，没有可见鼠标按钮。N 单击打开 16 色便签，选色一次放置，底部 Stack 连续放置。点空白创建并立即编辑，拖框按起止尺寸创建。空内容退出不保存、不进撤销；有内容后 Ctrl/Cmd+Enter 提交，Esc 取消。不计算、不改源 View",
                keys=("N",),
            ),
            QuickRow(
                "文字 Text",
            ...
            QuickRow(
                "形状 Shape",
                sub="S 单击打开形状与连接线目录：直线、箭头、折线与矩形/圆角矩形/椭圆/菱形/三角带名称选择。L 直达最近连接线。点空白创建默认 4×3，拖动定尺寸；Shift 保持比例，Alt 从中心，Cmd/Ctrl 暂停吸附。选中后图标栏改填充/描边/线宽/虚线/圆角。双击或选中后输入编辑内嵌文字。P 打开纵向画笔：钢笔/荧光笔/整笔擦除/套索和 3 个笔触预设，再点当前预设才改线宽和颜色",
                keys=("S",),
            ),
            QuickRow(
                "已有连线与笔画",
