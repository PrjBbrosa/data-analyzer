...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "便签 Sticky",
                sub="左侧创作段 V / N / T / S / L / P。N 进入便签。再点一次 Sticky 打开色板，不取消工具。点空白创建并立即编辑，拖框按起止尺寸创建。放置一次回选择；双击按钮或「固定连续创建」才连续。空内容退出不保存、不进撤销；有内容后 Ctrl/Cmd+Enter 提交，Esc 取消。不计算、不改源 View",
                keys=("N",),
            ),
            QuickRow(
                "文字 Text",
                sub="T 进入文字。点空白创建自适应宽文本框并立即编辑，拖动先定换行宽度。中文输入法组合期间快捷键不抢。选中或编辑时顶栏整框改字体/字号/B/I/U/对齐/列表/颜色/底色/链接，不伪装局部富文本。放置一次回选择；双击按钮或「固定连续创建」才连续。空内容退出不保存、不进撤销；非空提交一条历史。上限 6000 字",
                keys=("T",),
            ),
            QuickRow(
                "形状 Shape",
                sub="S 进入最近使用的闭合形状。再点一次 Shapes 打开 5 格：矩形、圆角矩形、椭圆、菱形、三角形。点空白创建默认 4×3，拖动定尺寸；Shift 保持比例，Alt 从中心，Cmd/Ctrl 暂停吸附。放置一次回选择；双击按钮或「固定连续创建」才连续。选中后 8 点改尺寸、顶栏改填充/描边/线宽/虚线/圆角（仅矩形类）并切换类型且保留框与文字。双击或选中后输入编辑内嵌文字，属于形状本身、不是文字对象；空文字仍保留形状。上限 6000 字。直线/箭头/折线不在形状里",
                keys=("S",),
            ),
            QuickRow(
                "连接线 Connector",
                sub="L 进入最近使用的直线、箭头或正交折线箭头。再点一次 Connector 打开 3 格切换类型。空白两击或按住拖放创建；从选中卡片或作者对象的东南西北锚点拖出。落到轮廓即结构化锚定，空白为自由端点；Shift 约束水平/垂直/45°，Cmd/Ctrl 暂停吸附与锚定。放置一次回选择；双击按钮或「固定连续创建」才连续。目标移动或改尺寸后端点仍在轮廓；删除或移到未放置时固化最后点并提示一次。不做自动避障、曲线或多折点",
                keys=("L",),
            ),
            QuickRow(
                "画笔 Draw",
                sub="P 进入画笔并保持连续，直到 V、Esc 或换工具。再点一次 Draw 打开非模态层：钢笔、荧光笔、橡皮擦、套索，不使用二级菜单。钢笔与荧光笔各 3 个颜色+线宽芯片，真实预览。橡皮擦为整笔擦除，轨迹碰到笔画即删除整条，一次扫过多笔为一条撤销；不删卡片或其他对象，不切分笔画。套索按对象中心选择卡片和作者对象，锁定对象仍可选但不随移动删除；Shift 追加，普通完成替换选择，完成后回选择，不写入、不进历史。按下到松开是一笔、一条撤销；空格/中键/右键平移时暂停采样，松开恢复画笔光标。Esc 取消当前未完成笔画或套索且不写入。荧光笔固定 35% 透明",
                keys=("P",),
            ),
            QuickRow(
                "多选 · 对齐",
