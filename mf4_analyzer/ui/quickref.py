...
QUICKREF: Tuple[QuickGroup, ...] = (
    ...
    QuickGroup(
        ...
        rows=(
            ...
            QuickRow(
                "自由网格",
                sub="新 Board 默认自由网格。左侧窄轨按钮切回模板；12 列是基准网格和导出标尺，不是日常拖动墙。工作区随内容扩展，四向可平移；拖到边缘自动平移并扩展画布。最多 24 张已放置、200 个 View。整数网格；直接拖动移动，选中后拖边角改尺寸；空白框选、Shift 点选多张；拖动预览最终落点与被推移卡（完整 ghost）；移动不改任何卡尺寸、缩放只改本卡；硬拒绝会说明原因和下一步，不弹框。导出 PNG 按已放置内容裁切，与适应内容同口径",
                keys=("Shift",),
            ),
            QuickRow(
                "键盘微调",
            ...
            QuickRow(
                "尺寸预设",
                sub="小 / 标准 / 宽 / 高 / 大 / 横幅在右键尺寸预设；「按原图比例」在右上角，把卡片缩到贴合预览图宽高比（只缩不放）；放不下则拒绝并保持原布局，不静默改邻卡。模板模式下该操作不可用",
                gesture="卡片右键 / 右上角",
            ),
            QuickRow(
                "撤销 / 重做",
            ...
            QuickRow(
                "画布缩放 / 平移",
                sub="每次打开或切换 Board 都自动适应内容。Ctrl+滚轮或触控板捏合以光标为锚，范围 25%–300%；空格+拖、中键拖或手掌四向平移；拖到边缘自动平移并扩展画布。工具栏 − / % / ＋ / 适应 / 100%。12 列基准网格是卡片 1× 标尺；导出 PNG 按已放置内容裁切，与适应内容同口径，缩放不改变卡片长宽比。适应内容把已放置卡片居中填满浮层安全区，最高 300%；100% 保持当前中心。双击卡片临时聚焦最高 300%，Esc 返回。≥60% 完整卡片，40%–59% 紧凑（藏页脚、留预览与类型），低于 40% 标题卡。整板概览仍可点卡片跳转。画布空白右击可适应、自动排版、复制或导出",
                keys=("Ctrl", "滚轮"),
            ),
            QuickRow(
                "源已变化",
