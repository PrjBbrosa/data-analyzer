...
DYNAMIC_OBJECT_NAMES: frozenset[str] = frozenset(
    {
        ...
        "frfInputDot",
        "frfOutputDot",
        # ultraview/chrome.py:416 ``f"ultraViewRail{short_name}Button"``
        # and :427 ``f"ultraViewRail{short_name}Badge"``, short_name ``Unplaced``
        # from ``_PANEL_SPECS`` (:377). Spec inventory cited :420 / :431.
...
def test_dynamic_construction_sites_match_this_head():
    ...
    assert 'object_name=f"ultraViewRail{short_name}Button"' in chrome[415]
    assert 'badge.setObjectName(f"ultraViewRail{short_name}Badge")' in chrome[426]
    assert '"Unplaced"' in chrome[376]
