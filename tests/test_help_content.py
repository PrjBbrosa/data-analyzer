...
def test_deck_data_valid_and_version_bumped():
    d = _deck_data()
    assert d["meta"]["version"] == "v8.2.5"
    assert d["meta"]["updated"] == "2026-09-16"
    assert d["meta"]["docVersion"] == "3.0"
    assert [c["v"] for c in d["changelog"]][:9] == [
        "v8.2.5", "v8.2.4", "v8.2.3", "v8.2.2", "v8.2.1", "v8.2.0", "v8.1.0", "v8.0.1", "v8.0.0",
    ]
    current_manual, _changelog = MANUAL.read_text(encoding="utf-8").split(
        '  "changelog": [', 1,
    )
    assert "TraceLab v8.2.5" in current_manual
...
def test_published_guide_tracks_v821_and_real_ui_assets():
    ...
    assert "TraceLab v8.2.5" in html
...
def test_panel_guides_cover_new_topics():
    ...
    for fname, kws in checks.items():
        ...
        assert "TraceLab v8.2.5" in text
        for kw in kws:
