...
def test_deck_data_valid_and_version_bumped():
    d = _deck_data()
    assert d["meta"]["version"] == "v8.4"
    assert d["meta"]["updated"] == "2026-09-24"
    assert d["meta"]["docVersion"] == "3.0"
    assert [c["v"] for c in d["changelog"]][:11] == [
        "v8.4", "v8.3.2", "v8.3.1", "v8.3.0", "v8.2.5", "v8.2.4", "v8.2.3", "v8.2.2", "v8.2.1", "v8.2.0", "v8.1.0",
    ]
    current_manual, _changelog = MANUAL.read_text(encoding="utf-8").split(
        '  "changelog": [', 1,
    )
    assert "TraceLab v8.4" in current_manual
