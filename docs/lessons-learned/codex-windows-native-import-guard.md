...
tests:
  - PYTHONPATH=. .venv/Scripts/python.exe -m pytest tests/test_native_import_boundaries.py -q
...
2026-05-22 update: `pya2l` is in the same crash class as `pyxcp` when imported inside an already-initialized PyQt process. Lazy import is not enough; public Cockpit/UI paths must call a subprocess wrapper, and only the child/in-process parse helper may import `pya2l`.
