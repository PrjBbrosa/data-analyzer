...
tests:
  - TMPDIR=/tmp QT_QPA_PLATFORM=offscreen PYTHONPATH=. .venv/bin/python -m pytest tests/ui/test_ultraview_capture.py::test_transient_overlays_hidden_but_markup_revision_is_captured tests/ui/test_ultraview_capture.py::test_idle_capture_coalesces_range_signals tests/ui/test_ultraview_capture.py::test_presentation_digest_pixel_affecting_field_matrix tests/ui/test_ultraview_mode_integration.py::test_idle_pan_and_markup_recaptures_time_preview -q
---
...
Rule: Digest must include `cursor_mode`, armed cursor x, and pill text fingerprint, or idle recapture will skip. Hide only dual-mode hover lines and `rbScaleBox`; keep armed single/dual lines and extreme markers. Grab through `grab_presentation_pixmap` (canvas + overlapping pill) at 1×. Coalesce pan/cursor/markup onto one single-shot idle timer; do not `request_capture` per `cursor_info`. Sheet hidden → no extra idle grabs. Cursor-move while settling may show stale「源已变化」, not missing.
