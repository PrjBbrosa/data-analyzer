...
status: active
owners: [codex]
keywords: [ultraview, free grid, out of grid, shrink, yield, FEEDBACK_OUT_OF_GRID, plan_layout, plan_neighbor_shrink]
...
Trigger: Changing UltraView free-grid drag/resize commit, `FEEDBACK_OUT_OF_GRID`, `plan_layout`, or `plan_neighbor_shrink`. (`plan_boundary_yield` was a zero-caller wrapper over `plan_layout` and was deleted on 2026-08-15.)
...
Verification: `test_plan_layout_edge_without_hole_rejects_without_grow_or_shrink`, `test_out_of_board_drop_rejects_wall_instead_of_shrinking_neighbors`, `test_free_grid_edge_drop_rejects_without_shrinking_neighbors`, `test_free_grid_out_of_bounds_move_toasts_without_commit`.
