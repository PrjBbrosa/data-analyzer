...
Rule: Stale keeps the last valid preview. Sync (`user-sync`) recaptures the live visible canvas and never recomputes DSP. Hidden source canvases must navigate-then-grab or toast; do not mark fresh when `current_digest` is missing. Orphaned cards use rebind, not sync. Batch/rail 「一键更新源」 reuses the same `sync_requested` → `sync_preview` path and must serialize navigate-needed items (and wait for that ref's queued grab) so the last navigate cannot steal an earlier canvas. Raise the UltraView sheet only after the nav queue is empty.
