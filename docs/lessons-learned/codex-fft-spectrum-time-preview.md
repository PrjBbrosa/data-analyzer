...
status: active
owners: [codex]
keywords: [fft, PgLineCanvas, plot_spectra, source-summary, time-preview, psd, cursor-line]
...
Rule: The FFT canvas top row overlays spectrum amplitude curves; the lower row is the current FFT input source time-domain preview and should overlay every checked channel before and after computation. Do not reintroduce a visible PSD row, `psd_y` inspector control, or passive hover cursor line. When navigator channels are checked, the inspector must show a source summary and hide the fallback single-signal combo.
