...
全局复制**图片**:用户入口 6 个 → 代码出口 2 个(其余 `clipboard()` 调用都是 `setText` 复制文字,在采集 Cockpit 侧,与本功能无关)。

1. **出口 1 = 图卡复制** `ChartStack._copy_card_image(card)`(`chart_stack.py:1378`):用画布无关的
...
2. **出口 2 = FFT-vs-Time 检查器复制** `MainWindow._copy_fft_time_image(mode)`(`main_window.py:2051`):
...
| 矢量图层模型 | `QGraphicsScene`:背景 `QGraphicsPixmapItem` + 各标注为独立 `QGraphicsItem`;完成时才拍平 | 支持选中/拖动/删除/重编辑/撤销重做;不在位图上「刷」 |
...
| v1 工具集 | 选择 / **裁剪** / 箭头 / 直线 / 矩形 / 画笔 / 文字 / **序号①②③**;颜色(6 预设)+ 粗细(3 档)+ 撤销重做删除 | 覆盖「裁/圈/指/写/编号」90% 场景 |
...
- 每个标注 = 独立 `QGraphicsItem`(z≥1):
  ...
  - 序号①②③:click 落自增编号圆点(圆+居中数字 group item);`self._next_index` 自增,删除不回收编号。
- 「选择」工具:item `ItemIsSelectable|ItemIsMovable`,可拖/改色/删;Del 删选中;非选择工具时背景不可选。
...
**工具栏(编辑器顶部一行,紧凑):**
```
[↖选择] [⛶裁剪] [↗箭头] [╱直线] [▭矩形] [✎画笔] [T文字] [①序号]  |  ●●●●●● 颜色  ─━▰ 粗细
                                          [另存为 Ctrl+S] [取消 Esc] [完成并复制 ↵]
```
- 颜色 6 预设(红/橙/黄/绿/蓝/黑或白),当前色高亮;粗细 3 档;`QUndoStack` 撤销/重做(Ctrl+Z/Y);
  工具状态机 `self._tool ∈ {select,crop,arrow,line,rect,pen,text,number}`。
...
- **编辑器图层**:各工具 press/move/release 生成对应 item 且数量正确;序号自增;选择工具下可选中、Del 删除;
