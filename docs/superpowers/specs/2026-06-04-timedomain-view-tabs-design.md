...
**Scope:** 在时域(time-domain)图表底部新增 Excel 风格的 **View 标签栏**,把"当前通道组合 + 颜色 + 模式 + 缩放 + 游标"等整套显示状态保存为可来回切换的 view(上限 6 个),并支持两个 view **并排对比**。本 spec 只覆盖时域;FFT / 阶次 / FFT-vs-Time 不在范围内。落盘(project 持久化)不在本期,但数据模型为其预留。
...
3. `bridge.apply(manager.get(j), navigator, time_card, canvas)`:
   ...
   - `time_card.set_plot_mode(state.plot_mode)`、`set_cursor_mode(state.cursor['mode'])`;
   - `canvas.plot_channels(ch_list, mode)` 重绘;
   - 恢复 `xlim`(走 `plot_channels_preserving_xlim` / `PgAxisHandle.set_xlim`)、`ylims`、游标位置、`axis_opts`;
   - `blockSignals(False)`。
...
- **UI 测(offscreen + qtbot)**:`ViewTabBar` 点击/双击改名/右键菜单/拖动各发对的信号且只发一次;`bridge.capture→apply` 往返保真(勾选集、plot_mode、xlim 一致);切到 FFT 模式标签栏隐藏。对标 `tests/ui/test_side_panel_widgets.py`。
...
4. **自动捕获时机**:任何切换前**必须先 `capture`**,否则丢未保存的缩放/游标。
