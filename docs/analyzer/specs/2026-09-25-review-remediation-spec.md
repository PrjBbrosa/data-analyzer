...
- 日期：2026-09-25。
- 状态：**D1–D8 已有实现证据；D9 的 fixture 引用滞留已修，失败身份与全量提速仍开放**。聚焦测试和本机 Cocoa 自动化场景已有证据；全量套件中断，Windows 包与 native 绘制未验收。不代表可以发布。历史执行见配套 plan 第 13 节，D9 执行见配套 plan 第 15 节。
...
Review 中通过的定向测试不证明无其他 bug。原修复范围为以上问题；2026-09-25 补充 D9，纳入测试效率及全量失败归因、确认的测试缺陷修复。原生窗口合成、Windows 包和全量 suite 的未验收状态单独保留。
...
7. 测试对象按用例释放，执行有预算且失败即时可追溯；测试误报与产品缺陷分别修复，提速不得削弱断言、隔离或覆盖。
...
| A10 / D9 | 当前观察失败均有 nodeid/阶段/快照/根因与回归证据；未知和历史证据缺口显式保留，真实 bug 不靠改断言过测 | 即时失败日志、分类台账、单例及触发组合 |
| A11 / D9 | item 完整 teardown 后 owner 可回收；异常路径也释放，真实 filter 泄漏仍失败，session 自有对象安全 | 独立子进程生命周期与泄漏反例 |
| A12 / D9 | 同条件有限对照给出耗时/内存证据；完整 gate 有界且清单无遗漏，分组不掩盖已知顺序污染 | 分阶段事件、对象/RSS、collection 覆盖及边界测试 |

功能实现、focused/boundary、全量集成、macOS 前台、Windows frozen 是不同状态列。所有适用列有证据后才能关闭对应问题；平台未运行时可报告“代码完成、原生验收待完成”，不能写整体完成。

## 12. D9 — 测试效率与失败治理合同（新增，待实施）

### 12.1 事实与未知

原全量约 168 分钟后中断，日志只有失败进度标记，约 50 项未有身份与堆栈。旧 lastfailed 缓存早于该运行，不能拿其 248 条记录代替本次名单。短时原生采样支持 GC 热点；小探针确认 UI fixture 的 item 属性强引用延长 Python owner 生命周期。这两者不证明所有失败是误报，也不证明全部耗时只有一个原因。

### 12.2 正确性与 ownership

- 测试设施 owner 负责 item 引用、Qt deferred delete、GC 与 filter 泄漏检查的完整生命周期。检查失败也必须清理；清理不能吞掉原始失败。下一用例前既无多余 owner 保留，也不误删 session 对象。
- 失败分类以有效产品合同和真实证据为准，允许一组根因影响多个测试。产品错误修 owner；过期断言修测试并保留替代覆盖；fixture/环境问题修边界；未知继续保留。单独通过但组合失败不是忽略理由。
- 不以固定顺序、自动重试、宽容差、无理由 skip/xfail、移除断言、白名单放宽或更改产品数值来美化通过率。合法平台条件跳过必须单列未验收。
- 性能改进保持 root collector 修复、QSettings/QSS 隔离、真实 Qt ownership、registry/heap 一致性与 raster admission 合同。GC/全堆扫描策略仅在测量与等效安全回归支持时调整。

### 12.3 执行与证据

- 复用现有 watchdog/selector。诊断短批次、串行、有硬预算、即时记录 nodeid/phase/失败栈，不用无界全量找第一批失败。超时、异常退出、maxfail 和未运行均不能称为完整通过。
- 每个新运行绑定 HEAD、相关 tracked/untracked 内容指纹和环境；执行过程中相关源码变化则结果 UNVERIFIED。历史 50 个标记不可恢复时保留批次级 UNKNOWN，用同范围新完整运行替代验收，不伪造逐条映射。
- 比较性能时固定 nodeid 与环境，仅允许声明中的优化补丁有差异；分别报告 setup/call/teardown、GC 与内存趋势。不得从一次小探针外推确定的全量提速倍数。
- 若采取顺序新进程分组，保存稳定 collection 清单与各组映射，完整集合无遗漏/重复；acquisition_ui 独立，保留必要同进程顺序污染回归。只跑聚焦组不等于全量。
- A10–A12 完成需要相应实证；原已通过的 A1–A9 聚焦记录不会自动赋予新增任务通过状态。最终平台证据边界保持不变。
