...
- 通道名称、数值、单位、Custom-X 方向及所属诊断文字均跟随通道颜色，色点使用同色；Δ 保留字重强调，不引入红涨绿跌。公共表头（含“方向”标题）、指标标签和省略摘要保持中性色。此规则按2026-09-13用户补充修订，适用于full/mini及全部布局；同一通道改色后立即刷新。
...
单游标 full 是名称/单位与当前值，mini 隐去名称保留色点、方向（若有）、值/单位。双游标 mini 与单游标一致，隐藏通道名及来源，只保留色点、单位、方向（若有）和一个优先指标；展开恢复名称，隐藏名称不再参与宽度预算（2026-09-13用户补充）。无开启统计项时保留身份/真实分支/诊断，不显示空数值表头。
