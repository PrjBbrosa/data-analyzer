...
## 7. 2026-09-03 勘误（仅更正 §3.2 / §3.8）

本勘误不重写 2026-08-31 的历史结论。当前实现中，§3.2 的 mini 指标优先级为
`Δ → Avg → Max → Min`；当差值启用时优先显示差值。§3.8 所称的全局偏好为六项，
其中新增 `show_delta_value`；最后来源关闭时该六项应用级偏好均不复位。
