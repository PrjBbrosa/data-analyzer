...
3. blocker 可以按稳定顺序平移到**拖拽轴优先**的合法位置，但不能被缩小，也不能让 mover 因碰到边界自动长大。

   > **2026-08-15 批注（review 裁决）**：本条原文写作「最近合法位置」，实现
   > （`free_grid.find_avoidance_rect` 的 docstring 与 `avoidance_preferred_delta`）
   > 一直是「沿拖拽轴优先，再按环形距离展开」，因此被推的卡可能落到比某个更近空位
   > 更远的位置。2026-08-15 全盘 review 裁决**接受实现语义**并按实现改写本条措辞，
   > 有界最近邻不在该批范围；作为补偿，displaced 卡完全落到可视区外时给
   > `logger.info` + 非模态提示（滚动跟随另议）。
...
7. 邻卡缩小、填满空隙、边缘自动扩展等能力只允许由显式“智能整理/整理布局”命令触发，并在提交前显示整体预览。
   ...
   > **2026-08-15 批注（现状）**：本条只落地了「普通手势不缩邻卡」这一半。
   > 缩邻卡的实现（`free_grid.plan_neighbor_shrink` + `LAYOUT_ARRANGE` 分支）
   > 已在库里，但**没有任何生产调用方**——UI 的「整理」走
   > `ultraview_state.organized_placements`（只压掉空行），不经 `plan_layout`，
   > 也没有提交前整体预览。代码作为预留保留并在 `free_grid.py` 就地标注，
   > 接线不在 2026-08-15 修复批范围。
