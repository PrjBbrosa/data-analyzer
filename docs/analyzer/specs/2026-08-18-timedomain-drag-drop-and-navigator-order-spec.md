...
- 日期：2026-08-18
- 状态：已实施；通道树文件排序入口补充于 2026-08-19
- 基线：`main@8383a7ee`
- 配套计划：
...
> “按各来源匹配同名通道”的自定义横坐标。排序只发生在左侧：拖文件卡片或通道树文件
> 根节点移动整个文件块，拖通道行改变该逻辑源内部顺序；画布内不提供曲线/分屏行拖动。分屏顺序严格投影左侧
...
2. 通道行拖到时域画布最底部横坐标带：将该通道名设为自定义横坐标，并同步右侧
   “图表设置（横坐标 · 时间范围）”。
3. 左侧文件卡片和通道树的文件根节点均可上下拖动排序；一个物理文件展开出的多个逻辑源
   作为一个文件块移动。
...
- 通道树只允许 top-level `file` / `source` 根节点作为文件拖动源和目标；raster/逻辑源子节点
  不能拆开物理文件块。树目标同样显示 2px before/after 插入线。
- 树端只携带一个 fid anchor；`FileNavigator` 把源/目标均扩展为同一个物理卡片的完整 fid
  块，再进入既有 `file_order_requested` → `NavigatorOrderState` 路径。
