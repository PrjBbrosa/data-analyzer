...
| 画布级 | 左侧 ToolRail、右上 GlobalIsland | View、布局、自由网格、筛选、未放置、显示、导出、演示 |
...
| View 库浮层 | 264～288 px 宽；高度跟列表内容走，上限为画布高度减 96 px |
| 左轨弹层（库/布局/筛选/未放置） | 贴 ToolRail 右侧、Board 岛下方打开 |
| 显示/导出弹层 | 贴 GlobalIsland 下方右对齐；高度跟内容走，不留 stretch 空白 |
| 布局弹层 | 240～300 px；八模板缩略图可完整显示中文标签 |
| 筛选弹层 | 约 160 px 宽；选项纵向排列 |
| 未放置浮层 | 320～520 px 宽；条目纵向全宽排列，最多 3 行后内部滚动 |
...
- **2026-08-15 批注（board-popover-and-trigger-state）**：Board 管理入口从「下拉 + hover 子菜单」改为「单层 CanvasHost overlay + 行拖拽排序」。排序能力保留，四条排序菜单项改为列表内拖拽。「切换到此 Board」与 hover 子菜单删除。点画布关面板不再把焦点还给触发按钮；Esc 仍还给。同日跟进：行尾改为复制/删除图标（去掉 ⋯ 菜单）；弹层高度随行数增长，新建后不再把上方 Board 卷出视野。重命名仍走双击名称或 F2。
...
- 浮层高度按当前展开内容计（下限约 320 px，上限为画布高度减 96 px）。section 标题与 View 行锁最小高度；超出上限时在浮层内滚动，不得把标题或行压成一条缝。
...
ToolRail 第二项为布局，弹层只展示八种模板，不照搬 HTML 的四个示意项：
...
每项用小型几何预览和文字标签，格高须容纳完整中文名称。

受控自由网格是 ToolRail 上布局与筛选之间的独立开关，不进入布局弹层列表。点击立即切换模式，不打开 overlay；checked / active wash 表示当前已是自由网格。切入自由网格后，打开布局弹层才显示“整理布局”、Undo、Redo；切回模板继续使用当前确认文案，并把超出容量的引用移入未放置区。选模板会退出自由网格。

布局切换、自由网格转换、整理和历史仍通过现有 Page intent 与 coordinator 单写点提交，不新增第二条 Board mutation 路径。

### 5.4 筛选弹层

ToolRail 漏斗在自由网格开关之下：

- 选项为全部、时间轴、频率轴、时频轴、阶次轴，**纵向全宽排列**。
...
- **显示**：Board 级开关“卡片标题”“来源文件”。弹层贴 GlobalIsland 下方右对齐。
- **导出**：复制整板图、PNG 1×、PNG 2×。弹层贴 GlobalIsland 下方右对齐。
- **演示**：进入/退出演示模式，不打开 overlay。
...
ToolRail 底部为未放置图标和数量 badge。点击后打开浮层，完整复用 UnplacedTray 内容和动作。条目纵向全宽排列（状态、名称、放置/移除），不再做成底部横滑条；超出三行后在浮层内滚动。
...
| UV-NR-A05 | 八种模板可达；自由网格由左轨独立开关切换；整理、Undo/Redo 在布局弹层且仅自由网格时出现；切回模板确认与溢出语义不变 |
