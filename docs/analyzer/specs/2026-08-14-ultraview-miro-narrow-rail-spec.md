...
- **2026-08-15 批注（board-popover-and-trigger-state）**：Board 管理入口从「下拉 + hover 子菜单」改为「单层 CanvasHost overlay + 行拖拽排序」。排序能力保留，四条排序菜单项改为列表内拖拽。行尾 ⋯ 点击出复制/重命名/删除。「切换到此 Board」与 hover 子菜单删除。点画布关面板不再把焦点还给触发按钮；Esc 仍还给。
