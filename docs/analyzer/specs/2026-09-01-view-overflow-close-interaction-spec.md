...
- 日期：2026-09-01
- 交互修订：2026-09-03（View 管理入口常驻；溢出时同一入口显示隐藏数量）
...
3. 现有溢出按钮升级为**常驻 View 管理入口**：全部标签放得下时显示 `⋯`，存在隐藏
   标签时在同一位置显示 `»N`；点击均展开同一个“全部 View”面板。每行可单独关闭，
   底部提供带精确数量的“关闭其他”和“关闭全部”。不增加第二个全局入口。
4. 底栏色标 `×` 与右键删除继续发出既有 `delete_requested`，因此时域的既有确认、
   分析 View 的既有清理逻辑和“至少保留一个 View”约束都不变。
5. “全部 View”面板是整理台：行内 `×` 直接关闭该项、面板保持打开并重投影列表；不弹
...
- 将现有溢出菜单升级为由常驻入口打开、可管理全部 View 的 popup。
...
- 不把底栏/右键的单项删除统一成新的确认策略；时域与分析区继续沿用各自当前语义。
  “全部 View”面板行内 `×` 是整理入口，直接执行、不叠加单删确认。
- 不允许工作区进入零 View 状态。
...
- 继续复用现有 `viewTabOverflow` 按钮和位置，不增加额外按钮。该入口始终可见：无隐藏
  View 时显示 `⋯`；有隐藏 View 时显示 `»N`，其中 `N` 只表示未显示在标签栏上的数量。
- tooltip/accessibility 必须表达“管理全部 View”；溢出态可同时说明隐藏数量，不得把
  `»N` 误写成总 View 数。
- 入口在 roomy、compact、overflow 三个测量阶段均按其最宽可能文案预留固定宽度；不得
  因 `⋯`/`»N` 切换产生 rail 抖动或 fit 往返。
- popup 锚定到该常驻入口的下边缘，并按可用屏幕空间向左/向上夹取；不得越出当前屏幕。
...
- 多 View 时按钮：`关闭其他 <N-1> 个…`
- 多 View 时按钮：`关闭全部 <N> 个…`
- 唯一 View 时保留不带 `0 个` 的简洁文案并 disabled。
...
- 点击行末 `×`：面板保持打开，发出 `overflow_delete_requested(index)`（不走
  ...
  确认。
- 删除后即使不再溢出，入口仍切回 `⋯`，popup 保持打开并重投影全部 View；只有切换
  View、批量操作、外点、`Esc` 或 owner teardown 才按既有合同关闭。
...
- 常驻入口（`⋯` / `»N`）可用 Space/Enter 打开；`Esc` 关闭并恢复焦点。
...
- popup 在无溢出和有溢出状态都列出全部 View；当前标记与 manager 一致，`»N` count
  只等于隐藏标签数。
