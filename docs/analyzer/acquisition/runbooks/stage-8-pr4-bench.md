...
2. Build the folder executable and keep its runtime-smoke JSON.
   ...
   .\dist\TraceLab7.8\TraceLab7.8.exe --acquisition-runtime-smoke --json docs\analyzer\acquisition\evidence\vector-xcp\packaged-runtime-smoke.json
   ```
   ...
   `TraceLab7.8.exe --a2l-probe-child`, return code zero, and the expected
