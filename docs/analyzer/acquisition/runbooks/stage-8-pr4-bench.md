...
2. Build the folder executable and keep its runtime-smoke JSON.
   ...
   .\dist\TraceLab8.2.1\TraceLab8.2.1.exe --acquisition-runtime-smoke --json docs\analyzer\acquisition\evidence\vector-xcp\packaged-runtime-smoke.json
   ```
   ...
   `TraceLab8.2.1.exe --a2l-probe-child`, return code zero, and the expected
