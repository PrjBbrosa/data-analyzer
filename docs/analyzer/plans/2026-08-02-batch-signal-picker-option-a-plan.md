...
真机截图路径：（未执行）
15 项核对结果：（未执行）
第 11 项（搜索零位移）：（未执行；offscreen 侧已由
  test_picker_trigger_geometry_is_stable_across_search 与
  tools/render_batch_input_output_polish.py 的越界/位移断言覆盖，
  但按 CLAUDE.md 不作为视觉验收依据）
```
