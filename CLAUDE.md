...
- View 上限按 manager 区分：时域 24（`ui/view_state.TIME_DOMAIN_MAX_VIEWS`；
  `window.py` 传 `max_views=TIME_DOMAIN_MAX_VIEWS`，读 `view_manager.max_views`），
  四个分析分区仍 12（`ui/view_state.MAX_VIEWS`）。窄宽度下 tab 先压成序号、
